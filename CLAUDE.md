...
**Last updated:** 2026-08-28 20:00 EDT (Phase 6 / Q3 email delivery **live** -- AWS SES, `PmtcMail` stack deployed and verified with a real send including confirmed inbox delivery; placeholder deck until Open Item #2 lands. App-side wiring still needs a `PmtcApp` redeploy, PROJECT_STATE.md Open Item #14b. Previously: 18:05 EDT merge of `klutetr/new-domain` into `main`: custom domain live at `https://k1x-pmtc.geniusdrive.com/`, plus Ben's Results-page work -- Peer Leaders score fix and Excel rounding parity, breadcrumb bidirectional nav, narrative redesign, maturity-curve label fixes)
...
> iframe embed and the wizard loses state. `AFTER_YOU_PULL.md` has the exact
> fix. Confirmed still unfixed as of this merge (`app/app/__init__.py` is
> untouched on both sides). Delete this banner once it is done.
...
**Model:** Profile (org name, industry, 6 goal priorities) -> Assessment (10 capability ratings, None/Ad-Hoc/Standardized/Automated/Integrated/Autonomous = 0-5) -> Calculation (per-capability score, overall average, peer comparison, goal-weighted priority-gap ranking) -> Results (score ring, maturity curve, strengths/opportunities, lead-capture modal) -> [future phase] PPTX/PDF report.
...
| `[TEMPLATE].pptx` | Output deck template for python-pptx generation | **Does not exist yet.** Phase 5/9 blocked until Ben provides one. |
...
| `Application\mailer\` | The report mailer: container Lambda, LibreOffice, SES. See its own `README.md` | **Live** (deployed 2026-08-28, verified end to end). The deck it fills is a placeholder until Open Item #2 lands |
...
**Pending.** No PPTX template exists in the project folder yet (only `K1x Discovery Guide v2 - June 30.pptx`, which is a sales deck, not a report template). Phase 5/9 (PPT generation) is blocked until Ben provides one. The Results page's "Get My Report" modal captures lead info now but nothing downloads yet.
...
| PPTX report generation deferred to its own phase | No template exists yet. Sequencing: build Phases 0-4 (live scored web flow) first, pick up Phase 5/9 once a template lands. Agreed with Ben 2026-08-26. |
...
| `Assess_*_advice` (per-capability, per-level "what to do next" text, `Data!B68:H80`) not wired into calculator.py or the three pages | Not displayed anywhere in the approved wireframe -- it's report content (`Q` column / named ranges), and the PPTX report phase is still blocked on Open Item #2 (no template). Revisit when that phase starts; the underlying `Data!B68:H80` table is fully read and documented in this session's transcript if it's needed sooner. 2026-08-27.
