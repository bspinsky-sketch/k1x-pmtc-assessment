...
- Removed the duplicate `</div>` from `output_report/03-how-scored.tmpl.html`'s zone-card block (the static `.html` sibling was already correctly structured -- only the Jinja2 template had the extra tag, meaning this bug affects any real report the live app/mailer generates through page 3, not just this one preview).
